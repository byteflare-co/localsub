...
| 主要エンドポイント | CLI起動時に最大24時間に1回 `GET https://api.github.com/repos/byteflare-co/localsub/releases?per_page=20`。Luna選択・同意時だけ `POST https://api.openai.com/v1/responses` |
...
| HTTPS | `https://api.github.com/repos/byteflare-co/localsub/releases?per_page=20` | CLIのバージョン通知。media由来情報なし。`LOCALSUB_NO_UPDATE_CHECK=1`で停止 |
